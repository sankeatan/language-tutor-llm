...
export function ChatUi() {
  ...
  const handleSend = async () => {
    ...
    if (input.trim() && currentConversation) {
      ...
      try {
        const response = await axios.post(`http://localhost:4000/chat/${currentConversation.id}`, {
          message: input
        })
  
        const aiMessage: Message = {
          role: 'ai',
          content: response.data.conversation.messages[1].content
        }
        ...
        setConversations(conversations.map((conv) => (conv.id === currentConversation.id ? updatedWithAIResponse : conv)));
      } catch (error) {
      ...
      //New Conversation
      updatedConversation = {
        id: conversations.length + 1, //Temp Id
      ...
      try {
        // Send message to the backend to create a new conversation
        const response = await axios.post('http://localhost:4000/chat/create', {
          userId: '66e85038e4b0eb4311efeb60',  
          messages: [{ role: 'user', content: input }],
        });
  
        const conversationData = response.data.conversation;
        const aiMessage: Message = conversationData.messages.find(
          (msg: Message) => msg.role === 'ai'
        );
  
        const updatedWithAIResponse = {
          ...updatedConversation,
          id: conversationData._id,  // Use the ID generated by the backend
          messages: [...updatedConversation.messages, aiMessage],
          lastUsed: new Date(),
        };
  
        setCurrentConversation(updatedWithAIResponse);
        setConversations([...conversations, updatedWithAIResponse]);
      } catch (error) {
